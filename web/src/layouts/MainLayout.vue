        ...
        <div class="header">
            <div class="header-item">
                <fluent-menu class="hidden-up" style="--menu-max-height: auto;">
                    ...
                    <fluent-listbox>
                        <gugu-menu-item v-for="tabData in tabDatas" :selected="tabData.key == activeId" @click="activeId = tabData.key">
                            <template #icon><component :is="tabData.svg" /></template>
                            {{ tabData.name }}
                        </gugu-menu-item>
                    </fluent-listbox>
                </fluent-menu>
            ...
            <div class="header-item">
                ...
                <GuguLangSwitch />
                <fluent-button appearance="transparent" icon-only shape="circular" :title="$t('theme.switch')" @click="switchTheme">
                    <svg width="24" height="24" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
...
.hidden-down { display: block; }
.hidden-up { display: none; }
@media (max-width: 480px) {
    .hidden-down { display: none; }
    .hidden-up { display: block; }
    .tab-router-view { margin: 16px; }
}
fluent-tab { margin: 0 8px; }
.tab-title { min-width: max-content; }
.fade-enter-active, .fade-leave-active { transition: opacity 0.3s ease; }
.fade-enter-from, .fade-leave-to { opacity: 0; }
.slide-left-enter-active { transition: all 0.15s cubic-bezier(0, 1, 1, 1); }
.slide-left-leave-active { transition: all 0.15s cubic-bezier(1, 0, 1, 0); }
...
.slide-right-enter-active { transition: all 0.15s cubic-bezier(0, 1, 1, 1); }
.slide-right-leave-active { transition: all 0.15s cubic-bezier(1, 0, 1, 0); }
...
fluent-drawer-body::part(content) { overflow: hidden; }
</style>
