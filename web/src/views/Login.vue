    ...
    <transition name="scale" mode="out-in" appear>
      <form v-if="currentCard === 'login'" class="card" @submit.prevent="onLogin" @keydown.enter.prevent>
        ...
        <fluent-field label-position="above">
          ...
          <fluent-text-input slot="input" required type="email" spellcheck="false" autocomplete="username"
        ...
        <fluent-field label-position="above">
          ...
          <fluent-text-input slot="input" required type="password" autocomplete="current-password"
      ...
      <div v-else-if="currentCard === 'register'" class="card">
        ...
        <form v-else style="width:100%;display:flex;flex-direction:column;align-items:center;gap:16px;" @submit.prevent="onRegister" @keydown.enter.prevent>
          <label class="title">{{ $t('login.registerTitle') }}</label>
          <fluent-field label-position="above">
            <label class="hide-required" slot="label">{{ $t('login.email') }}</label>
            <fluent-text-input slot="input" required type="email" spellcheck="false" autocomplete="off"
          ...
          <fluent-field label-position="above">
            <label class="hide-required" slot="label">{{ $t('login.password') }}</label>
            <fluent-text-input slot="input" required pattern="^(?=.{6,20}$)[\x21-\x7E]+$" type="password"
      ...
      <form v-else-if="currentCard === 'resetPassword'" class="card" @submit.prevent="onResetPasswordSendEmail"
        ...
        <fluent-button appearance="primary" type="submit" :disabled="resetPasswordSendEmailLoading || resetPasswordSendEmailSuccess">
